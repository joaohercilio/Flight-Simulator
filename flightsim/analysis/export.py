...
from flightsim.core.dynamics import FORCE_NAMES
...
def _force_column(name: str) -> str:
    if name in ("nx", "ny", "nz"):
        return name
    return name + ("_Nm" if name.split("_")[0] in ("l", "m", "n") else "_N")


FORCE_COLUMNS = [_force_column(n) for n in FORCE_NAMES]
...
def export_csv(res: SimulationResult, path: pathlib.Path, t_start: float | None = None, t_end: float | None = None) -> int:
    ...
    columns = (["time_s"] + STATE_COLUMNS + [c + "_dot" for c in STATE_COLUMNS[6:]]
               + ["alpha_rad", "beta_rad", "airspeed_m_s"] + CONTROL_COLUMNS + FORCE_COLUMNS)
    data = np.column_stack([res.t, res.x.T, res.dx[StateIndex.U:StateIndex.R + 1].T,
                            res.alpha, res.beta, res.airspeed, res.u.T, res.f.T])
