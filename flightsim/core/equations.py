def navigation_equations(u, v, w, sphi, cphi, stet, ctet, spsi, cpsi):
    xdot = u*ctet*cpsi + v*(sphi*stet*cpsi - cphi*spsi) + w*(cphi*stet*cpsi + sphi*spsi)
    ydot = u*ctet*spsi + v*(sphi*stet*spsi + cphi*cpsi) + w*(cphi*stet*spsi - sphi*cpsi)
    zdot = -u*stet + v*sphi*ctet + w*cphi*ctet
    return xdot, ydot, zdot


def kinematic_equations(p, q, r, sphi, cphi, ctet, ttet):
...
def translational_equations(mass, g, X, Y, Z, u, v, w, p, q, r, sphi, cphi, stet, ctet):
...
def rotational_equations(Ix, Iy, Iz, Ixz, l, m, n, p, q, r):
    den = Ix*Iz - Ixz**2
