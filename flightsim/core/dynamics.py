...
This replaces the old ``make_state_eq`` factory. ``Dynamics`` is an
object so the GUI can hold one instance, swap the control source, and
call it once per integration step. It implements ``__call__(x, t)`` so
it drops straight into the existing integrators (rk4, rk4_step).
"""
...
class Dynamics:
    ...
    Args:
        ...
        controls: Control source queried each step for commands.
    ...
    def __init__(
        ...
        # ---------------------------------------------------------------
        self.POS_MG    = 0.44
        self.WHEELBASE = 0.306  # This is your B_mg
        self.Y_MG      = 0.2
        self.Z_GEAR    = 0.15
        ...
        self.max_brake    = model.brake_max
    ...
    def __call__(self, raw: NDArray, t: float) -> NDArray:
        ...
        Args:
            raw: State vector, shape (12,).
            t: Simulation time (s), passed to the control source.

        Returns:
            State derivative, shape (12,).
        """
        model        = self._model
        ...
        ele_cmd, ail_cmd, rud_cmd, throttle_cmd, brake_cmd = self._controls.get(t).as_tuple()
